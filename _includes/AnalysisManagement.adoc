...
image:AnalysisManagement_2_800.png[Analysis Management esit options]
    ...
    This option allows to modify the name, the description of the analysis and add or remove linked referential. The Edit risk analysis pop-up appears:
...
Important note [! - exclamation mark image]: Analysis can be only imported when the language of its creation is the same as the current analysis (into which import is performed).
...
- The export file can be protected by password if set (Custom password option).
