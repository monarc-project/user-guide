...
--
The operational risk table now includes a read-only `Risk source` column between `Asset` and `Risk description`.
It keeps the origin of the scenario visible directly in the operational risk register and displays `-` when no source has been assigned yet.
--
...
.ISO 27005:2022 adaptation
--
When the operational risk's `Next reassessment date` is in the past, an orange warning icon is displayed next to its identifier. Hover the icon to see the overdue date.
--
...
.ISO 27005:2022 adaptation
--
The operational risk sheet now includes a `Risk source` row above `Risk description`.
...
.ISO 27005:2022 adaptation
--
The operational risk sheet now follows the same card structure as the information risk sheet:
...
The `Governance & review` card contains the supervisor-based `Risk owner` field together with `Last review date`, `Next reassessment date`, `Related reassessment trigger criteria`, and `Review frequency`.
When a last review date is set, MONARC proposes a next reassessment date one year later. You may change that date. Related trigger criteria become available after a next reassessment date is set and link the operational risk to one or more analysis-level trigger criteria.

The risk-level review frequency can be `Monthly`, `Quarterly`, `Semi-annually`, `Annually`, `On trigger`, or `Other` with a custom value. It records the intended rhythm; the warning icon is controlled by the explicit next reassessment date.
The `Residual acceptance` card captures the residual-risk approver, decision, date, performer, and justification.
The `History` card exposes a read-only audit trail for the current operational risk.
--
