...
--
The information risk table now includes a read-only `Risk source` column after `Asset`.
It shows the selected source label for each risk or `-` when no source has been assigned yet.
--
...
.ISO 27005:2022 adaptation
--
When the risk's `Next reassessment date` is in the past, MONARC displays an orange warning icon next to its identifier. Hover the icon to see the overdue date.
The warning is based on the explicit next reassessment date; it is not a risk-level calculation from `Review frequency` alone.
--
...
.ISO 27005:2022 adaptation
--
The information risk sheet now includes a `Risk source` row above `Threat`.
From this field, you can select an active source, choose `None`, clear the current value, or create a new source directly from the sheet and assign it immediately.
--
...
.ISO 27005:2022 adaptation
--
The information risk sheet is now organised into dedicated cards:
...
The `Governance & review` card groups the supervisor-based `Risk owner` field with `Last review date`, `Next reassessment date`, `Related reassessment trigger criteria`, and `Review frequency`.
Set a last review date to automatically prefill the next reassessment date to one year later. You can retain that date or select another date. A related trigger criterion can be selected only after a next reassessment date has been set; select one or more criteria to show which analysis-level triggers apply to this risk.

The available risk-level review-frequency values are `Monthly`, `Quarterly`, `Semi-annually`, `Annually`, `On trigger`, and `Other`. `Other` accepts a custom label. The review frequency documents the intended rhythm; the overdue warning for a risk is driven by the next reassessment date.
The `Residual acceptance` card records the approver, decision, date, performer, and justification of the residual-risk decision after treatment.
The `History` card provides a read-only audit trail of changes for the current risk.
--
...
The risk source can then be assigned afterward from the risk information sheet.
